...
operDef
    : qualifier normalCallName
        ...
        ':' type
        // We support header declaration with no implementation for documentation generation
        ('=' expr)?
...
wrongTypeApplication : typeCtor=qualId '(' typeArgs ')' {
        notifyErrorListeners("QNT009: Use square brackets instead of parenthesis for type application: "
                  + $qualId.text + "[" + $typeArgs.text + "]");
      };
...
expr:           // apply a built-in operator via the dot notation
        ...
        |       expr '=' expr {
                  notifyErrorListeners("[QNT006] unexpected '=', did you mean '=='?");
                }                                                   # errorEq
...
simpleId[String context]
    : identifier
    | qualId {
        notifyErrorListeners("QNT008: Identifiers in a " + $context + " cannot be qualified with '::'. Found " + $qualId.text + ".");
      }
    ;
...
identifier : LOW_ID
           ...
           | keywordAsID
           | reserved {
               notifyErrorListeners("QNT008: Reserved keyword '" + $reserved.text + "' cannot be used as an identifier.");
             };
